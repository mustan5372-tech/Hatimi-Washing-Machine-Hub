...
import { Phone, MapPin, MessageCircle, Clock, ShieldCheck, Lock, Search, FileText, CheckCircle2 } from 'lucide-react';
import type { BusinessSettings, SaleRecord } from '../../types';
...
export const PublicCustomerPortal: React.FC<PublicCustomerPortalProps> = ({
  ...
  const [foundInvoice, setFoundInvoice] = useState<SaleRecord | null>(() => {
    if (!activeInvoiceNumber) return null;
    return sales.find(s => s.invoiceNumber.toLowerCase() === activeInvoiceNumber.toLowerCase()) || null;
  });
  ...
  const handleSearchInvoice = (e: React.FormEvent) => {
    ...
    const matched = sales.find(
      s => s.invoiceNumber.toLowerCase() === searchInvoiceQuery.trim().toLowerCase() ||
           s.customerPhone.includes(searchInvoiceQuery.trim())
    );
    setFoundInvoice(matched || null);
  };
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans flex flex-col antialiased selection:bg-teal-500 selection:text-white">
      ...
      <main className="flex-1 max-w-5xl mx-auto w-full px-4 py-8 space-y-8">
        ...
        {/* If Customer is viewing an Invoice */}
        {foundInvoice ? (
          <div className="space-y-4">
            <div className="flex items-center justify-between bg-slate-900 border border-slate-800 p-3 rounded-2xl">
              <div className="flex items-center gap-2 text-xs text-teal-400 font-bold">
                ...
                Customer Tax Invoice Verification Portal
              </div>
              {onClearInvoiceQuery && (
                <button
                  onClick={() => {
            ...
            {/* Render Customer Invoice View in read-only public mode */}
          ...
          <>
            ...
            <section className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-4">
              ...
              <p className="text-xs text-slate-400">
                Received an invoice link or purchase receipt? Enter your Invoice Number below to view or print your receipt:
              </p>

              <form onSubmit={handleSearchInvoice} className="flex flex-col sm:flex-row gap-2 max-w-lg">
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    placeholder="e.g. INV-2026-0001 or Phone number"
              ...
              {searchInvoiceQuery && !foundInvoice && (
                <p className="text-xs text-amber-400 pt-1">
                  No invoice found matching "{searchInvoiceQuery}". Please check your invoice number or contact store support.
                </p>
              )}
            </section>
